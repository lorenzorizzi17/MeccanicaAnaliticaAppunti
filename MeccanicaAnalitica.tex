...
\begin{figure}[H]
	\centering
	\includegraphics[scale = 0.2]{ell1.jpg}
...
\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale = 0.2]{ell2.jpg}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale = 0.2]{ell4.jpg}
...
ossia la trottola deve spinnare su se stessa abbastanza velocemente (in fondo $p = I_z\dot\varphi$). Nelle situazioni reali, l'attrito tende a rallentare la rotazione finché l'equilibrio diventa instabile: la trottola si risveglia e segue la trattazione vista in precedenza.
	...
	Calcoliamo il tensore di inerzia di un rettangolo a densità omogenea. Siano $a$ e $b$ le due dimensioni lineari del rettangolo
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[x=1cm,y=1cm]
		\draw[->] (-2.5,0) -- (2.5,0) node[right] {$x$}; % x-axis
		\draw[->] (0,-2.5) -- (0,2.5) node[above] {$y$}; % y-axis
		
		\draw (-2,-1.5) rectangle (2,1.5); % rectangle

		
		
		\draw[dashed] (-2.5,0) -- (2.5,0); % dashed x-axis extension
		\draw[dashed] (0,-2.5) -- (0,2.5); % dashed y-axis extension
	\end{tikzpicture}
\end{figure}
Già sappiamo che $I_z = I_x+I_y$, quindi procediamo a calcolare:
$$
I_x = \int_{A}y^2\sigma dA =\sigma\int_{-b/2}^{b/2}dx\int_{-a/2}^{a/2}y^2dy = \dfrac{M}{ab}b\dfrac{1}{12}a^3 = \dfrac{1}{12}Ma^2
$$
Analogamente,
$$
I_y = \dfrac{1}{12}Mb^2
$$
e allora il tensore di inerzia si scrive nel sistema degli assi principali come:
\begin{equation}\label{key}
	I = \dfrac{1}{12}M
	\begin{pmatrix}
		a^2 & 0 & 0 \\
